...
export function CurrentHireTaxiSection() {
  const params = useParams();
  const hire_id = Number(params.id);
  const { data: hire_details } = useHireDetails(hire_id);
  const { taxi_id } = hire_details;
  const { data: taxi_details } = useTaxiDetails(taxi_id);
  const { data: licence_details } = useTaxiLicenceDetails(taxi_id);
